...
#    C = np.copy(A)
    ...
    n_Cho_npl = Resol_linalg_Cholesky(A,B)
    stop_Cho_npl = time.time()
    t_Cho_npl.append(stop_Cho_npl - start_Cho_npl)
    E_Cho_npl.append(np.linalg.norm(np.dot(A,n_Cho_npl)-np.ravel(B)))
